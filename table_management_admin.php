...
$current_category_id = isset($_GET['table_category']) ? intval($_GET['table_category']) : null;
$categories_result = $connection->query("SELECT * FROM table_categories");
$categories = $categories_result->fetch_all(MYSQLI_ASSOC);

$tables_zone = [];
if ($current_category_id !== null) {
    $stmt = $connection->prepare("SELECT * FROM tables WHERE table_categories = ?");
    $stmt->bind_param("s", $current_category_id);
    $stmt->execute();
    $success = True;
    $$table_result = $stmt->get_result();
    $tables_zone = $table_result->fetch_all(MYSQLI_ASSOC);
    $stmt->close();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  if (isset($_POST['add_table'])) {
      ...
      $stmt->bind_param("ssiss", $table_number, $table_category, $description, $active_bool);
      $stmt->execute();
      $stmt->close();
  }
}
  ...
  <div id="sidebar" class="sidebar">
    ...
    <?php endif; ?>
        ...
        <div style="display: flex; gap: 12px; overflow-x: auto;">
            <?php foreach ($tables_zone as $tbz): ?>
                <a href="table_management_admin.php?category=<?= $tbz['id'] ?>"
                    style="
                        padding: 8px 14px;
                        border-radius: 18px;
                        text-decoration: none;
                        white-space: nowrap;
                        background-color: <?= ($current_category_id == $tbz['id']) ? '#007bff' : '#e0e0e0' ?>;
                        color: <?= ($current_category_id == $tbz['id']) ? 'white' : '#333' ?>;
                        font-weight: <?= ($current_category_id == $tbz['id']) ? 'bold' : 'normal' ?>;
                    ">
                    <?= htmlspecialchars($tbz['name']) ?>
                </a>
            <?php endforeach; ?>
        </div>
        ...
        <div class="table-grid" style="display: flex; flex-wrap: wrap; margin-top: 80px; gap: 16px; padding: 20px;">
            <?php foreach ($tables_zone as $tables_zone): ?>
                <div class="product-card"
                    onclick="window.location.href='table_infp.php?id=<?= $tables_zone['id'] ?>'"
                    ...
                    <div style="padding: 10px;">
                        <h4 style="margin: 0 0 8px;"><?= htmlspecialchars($tables_zone['name']) ?></h4>
                    </div>
                </div>
            <?php endforeach; ?>
        </div>
