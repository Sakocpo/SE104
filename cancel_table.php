...
// 1) fetch that table’s current_order_id
...
if (!empty($row['current_order_id'])) {
    ...
    // 2) delete its items
    $del = $connection->prepare("DELETE FROM order_items WHERE order_id = ?");
    $del->bind_param("i", $order_id);
    $del->execute();
    $del->close();

    // 3) delete the order itself
    $del = $connection->prepare("DELETE FROM orders WHERE id = ?");
    $del->bind_param("i", $order_id);
    $del->execute();
    $del->close();

    // 4) clear the table’s pointer
    $upd = $connection->prepare("UPDATE tables SET current_order_id = NULL WHERE id = ?");
